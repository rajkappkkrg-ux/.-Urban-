...
<div class="container">
    ...
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:center;gap:16px;margin-bottom:20px;">
        ...
        <div style="text-align:left;">
          <div id="uName" class="username">Имя пользователя</div>
          <div id="uActivity" class="activity">Уровень активности: 65%</div>
        </div>
      </div>
      <div class="progress-bar"><div id="progress" class="progress-fill" style="width:65%"></div></div>
      <div id="uBalance" class="balance-block">2 000 ₽</div>
      <button class="btn btn-primary" onclick="showWithdrawal()">Вывести средства</button>
    </div>
    <div class="support">
      Связь с администрацией<br><br>
  ...
  <div id="withdrawalView" class="hidden">
    <h1>URBAN Акции</h1>
    <div id="wdAct" class="activity">Ваш уровень активности: 65%</div>

    <div style="font-size:1.25rem; line-height:1.55; margin:30px 0; color:#cbd5e1;">
      Вывод доступен только при уровне активности <span id="needAct">100</span>%.
    </div>

    <div style="font-size:1.18rem; line-height:1.6; margin-bottom:40px;">
      Для того чтобы получить <span id="bonusPct">35</span>% активности,<br>
      приобретите акций на <span id="needRub">5000</span> ₽.
    </div>

    <button class="btn btn-disabled">
      <span style="font-size:1.4rem; margin-right:8px;">🔒</span> Вывод
    </button>

    <button class="btn btn-back" onclick="backToMain()">← Назад</button>
  </div>

  <!-- Админ-панель (как было раньше, но с новыми полями) -->
  <div id="adminPanel" class="screen hidden">
    <h1>Админ-панель</h1>

    <label>Выберите пользователя или создайте нового</label>
    <select id="userSelect" onchange="loadUser()">
      <option value="-1">— Новый пользователь —</option>
    </select>

    <h3>Данные пользователя</h3>
    <input type="hidden" id="editIdx">
    <input type="text" id="name" placeholder="ФИО / Имя">
    <input type="number" id="balance" placeholder="Баланс ₽" min="0">
    <input type="number" id="activity" placeholder="Активность %" min="0" max="100">

    <h3>Настройки страницы вывода (для всех)</h3>
    <input type="number" id="needPercent" placeholder="Нужный % для вывода" min="1" max="100" value="100">
    <input type="number" id="bonusPercent" placeholder="% за покупку акций" min="1" max="100" value="35">
    <input type="number" id="needRubles" placeholder="Сумма покупки акций (₽)" min="100" value="5000">

    <button class="btn btn-success" onclick="saveAll()">Сохранить всё</button>

    <h2>Список пользователей</h2>
    <table id="tbl">
      <thead><tr><th>Имя</th><th>Баланс</th><th>%</th><th>Действия</th></tr></thead>
      <tbody></tbody>
    </table>

    <button class="btn btn-back" onclick="backToMain()">Закрыть админку</button>
  </div>
...
<script>
  ...
  // 4 быстрых тапа по аватару → админка
  ...
  function renderTable() {
    ...
    users.forEach((u, i) => {
      ...
      tr.innerHTML = `
        <td>${u.name}</td>
        <td>${Number(u.balance).toLocaleString('ru-RU')} ₽</td>
        <td>${u.activity}%</td>
        <td style="white-space:nowrap;">
          <button class="btn btn-edit" onclick="edit(${i})">Редактировать</button>
          <button class="btn btn-delete" onclick="del(${i})">Удалить</button>
          <button class="btn btn-enter" onclick="enterUser(${i})">Зайти</button>
        </td>`;
  ...
  function renderSelect() {
    ...
    select.innerHTML = '<option value="-1">— Новый пользователь —</option>';
    users.forEach((u, i) => {
  ...
  function enterUser(i) {
    ...
    document.getElementById('uActivity').textContent = `Уровень активности: ${u.activity}%`;
    document.getElementById('uBalance').textContent = `${Number(u.balance).toLocaleString('ru-RU')} ₽`;
    document.getElementById('progress').style.width = `${u.activity}%`;
    document.getElementById('wdAct').textContent = `Ваш уровень активности: ${u.activity}%`;
    document.getElementById('adminPanel').classList.add('hidden');
    document.getElementById('mainView').classList.remove('hidden');
  }
  ...
  function saveAll() {
    ...
    // Сохраняем пользователя (если есть имя)
    if (name) {
    ...
    // Сохраняем настройки страницы вывода
    ...
    // Применяем настройки на странице вывода
